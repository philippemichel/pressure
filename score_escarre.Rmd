...
rocph(sc1,"Score sans pondération", 0:4)
```
...
sc2 = (ttc$dj == "oui") * 5 +
  (ttc$crp == "oui") * 5 +
     (ttc$poids == "haut") * 4  +
     (ttc$corticoïdes == "oui") * 7 + 
     (ttc$déficit.neuro == "oui") * 9
valscore(sc2, "Score avec pondération")
```    
...
ggplot(zz) +
  ...
  theme(plot.title = element_text(size = 12, face="bold"),
        ...
        axis.text.y = element_text(size = 12),
        legend.position = "none")
rocph(as.numeric(tt$classification.risque),"Échelles habituelles",1:4)
```
